		...
		<div className="flex flex-col lg:flex-row min-h-screen bg-[#fdfbf7]">
			...
			<div className="flex-1 flex flex-col min-w-0">
				<header className="flex items-center justify-between px-4 sm:px-8 h-16 bg-white border-b border-[#e7e5e4] shrink-0">
					<h1 className="font-outfit text-[20px] font-semibold text-[#1c1917]">Modules</h1>
				...
				<main className="flex-1 p-4 sm:p-6 lg:p-8 flex flex-col gap-6 overflow-y-auto">
					...
					<div className="flex items-center justify-between flex-wrap gap-3">
						<h2 className="font-outfit text-2xl font-semibold text-[#1a3a2a]">Training Modules</h2>
						<button className="bg-[#1f4d35] text-white font-outfit text-sm font-medium py-2.5 px-5 rounded-lg border-none cursor-pointer transition-colors duration-200 hover:bg-[#1a3a2a]" onClick={() => navigate('/modules/new')}>
							+ New Module
						</button>
					</div>
					...
					{isLoading && <p className="text-center py-16 font-outfit text-sm text-[#a8a29e]">Loading modules…</p>}
					{error && <p className="text-center py-16 font-outfit text-sm text-red-500">Failed to load modules.</p>}
					...
					{!isLoading && !error && (
						filtered.length > 0 ? (
								...
								{filtered.map(m => (
									<div key={m.id} className="bg-white border border-[#e7e5e4] rounded-xl p-5 flex flex-col gap-3 transition-[box-shadow,transform] duration-200 hover:shadow-[0_4px_16px_rgba(26,58,42,0.1)] hover:-translate-y-0.5">
										<div className="flex items-center justify-end">
											<span className={`inline-flex items-center py-0.75 px-2.5 rounded-full font-outfit text-[11px] font-medium ${BADGE_CLASSES[m.status] ?? ''}`}>{m.status}</span>
										</div>
										<h3 className="font-serif text-base font-normal text-[#1a3a2a] leading-[1.5] flex-1">{m.title}</h3>
										<div className="flex gap-2 mt-auto pt-3 border-t border-[#f5f5f4]">
											<button
												className="flex-1 py-1.75 px-0 border border-[#1a3a2a] rounded-md bg-transparent font-outfit text-xs font-medium text-[#1a3a2a] cursor-pointer transition-all duration-150 hover:bg-[#1a3a2a] hover:text-white"
												onClick={() => navigate(`/modules/${m.id}/edit`)}
											>
												Edit
											</button>
											<button
												className="flex-1 py-1.75 px-0 border border-[#e7e5e4] rounded-md bg-[#f3faf6] font-outfit text-xs font-medium text-[#1a3a2a] cursor-pointer transition-colors duration-150 hover:bg-[#e8f5ee]"
							...
							<div className="text-center px-6 py-16 font-serif text-base text-[#a8a29e]">No modules found.</div>
						)
					)}
