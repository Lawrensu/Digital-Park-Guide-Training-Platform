	...
	if (error || !attempt) {
		return (
			<div className="flex flex-col lg:flex-row min-h-screen bg-[#fdfbf7]">
				<Navbar />
				<div className="flex-1 flex items-center justify-center">
					<p className="font-outfit text-sm text-red-500">Failed to load quiz attempt.</p>
				</div>
			</div>
		)
	}
		...
		<div className="flex flex-col lg:flex-row min-h-screen bg-[#fdfbf7]">
			...
			<div className="flex-1 flex flex-col min-w-0">
				<header className="flex items-center justify-between px-4 sm:px-8 h-16 bg-white border-b border-[#e7e5e4] shrink-0">
					<h1 className="font-outfit text-[20px] font-semibold text-[#1c1917]">Certifications</h1>
					<div className="flex items-center gap-3">
						...
						<div className="w-9 h-9 rounded-full bg-[#2d7d4e] flex items-center justify-center font-outfit text-xs font-semibold text-white">{initials}</div>
					</div>
				</header>
				...
				<main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
					...
					<div className="flex items-center gap-4 mb-8">
						...
						<h2 className="font-outfit font-semibold text-[28px] text-[#1a3a2a] m-0">Issue Certificate</h2>
					</div>
					...
					<div className="flex flex-col lg:flex-row gap-8 items-start max-w-5xl">
						...
						<div className="flex-1 space-y-6">
							...
							<div className="flex items-start gap-3 bg-[#e8f5ee] border border-[#a7d9b8] text-[#1a3a2a] p-4 rounded-xl">
								...
								<div>
									<span className="font-outfit font-semibold text-sm">Quiz passed ({score}%)</span>
									<span className="font-outfit text-sm text-[#266841]"> — fill in the certificate details and issue.</span>
								</div>
							</div>

							<div className="bg-white border border-[#f0e9db] rounded-xl p-6">
								<h3 className="font-outfit font-medium text-[20px] text-[#1a3a2a] mb-6 border-b border-[#f0e9db] pb-3">Attempt Details</h3>

								<div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
									<div className="flex flex-col gap-1">
										<label className="font-outfit font-medium text-sm text-gray-500">Guide</label>
										<p className="font-serif text-base text-[#1a3a2a] font-medium m-0">{guideName || '—'}</p>
									</div>
									<div className="flex flex-col gap-1">
										<label className="font-outfit font-medium text-sm text-gray-500">Module</label>
										<p className="font-serif text-base text-[#1a3a2a] m-0">{courseName}</p>
									</div>
									<div className="flex flex-col gap-1">
										<label className="font-outfit font-medium text-sm text-gray-500">Quiz</label>
										<p className="font-serif text-base text-[#1a3a2a] m-0">{attempt.quiz?.title ?? '—'}</p>
									</div>
									<div className="flex flex-col gap-1">
										<label className="font-outfit font-medium text-sm text-gray-500">Score</label>
										<p className="font-serif text-base text-[#1a3a2a] m-0">{score}%</p>
									</div>
								</div>
							</div>

							<div className="bg-white border border-[#e7e5e4] rounded-xl p-6">
								<h3 className="font-outfit font-medium text-[20px] text-[#1a3a2a] mb-6 border-b border-[#e7e5e4] pb-3">Certificate Details</h3>

								<div className="grid grid-cols-2 gap-5">

									<div className="flex flex-col gap-1.5 col-span-2">
										<label className="font-outfit font-medium text-sm text-[#44403c]">
											Issuing Organisation <span className="text-red-500">*</span>
										</label>
										<input
											type="text"
											className="py-2.25 px-3.5 border border-[#e7e5e4] rounded-lg bg-[#fafaf9] font-outfit text-sm text-[#1c1917] transition-[border-color] duration-150 focus:outline-none focus:border-[#1a3a2a] focus:bg-white"
											value={companyName}
									...
									<div className="flex flex-col gap-1.5">
										...
										<input
											type="date"
											className="py-2.25 px-3.5 border border-[#e7e5e4] rounded-lg bg-[#fafaf9] font-serif text-sm text-[#1c1917] transition-[border-color] duration-150 focus:outline-none focus:border-[#1a3a2a] focus:bg-white"
											value={issueDate}
								...
								{issueError && <p className="font-outfit text-xs text-red-500 mt-4">{issueError}</p>}

								<div className="flex gap-3 mt-6">
									<button
										onClick={() => issueMutation.mutate()}
										disabled={issueMutation.isPending || !canSubmit}
										className="py-2.5 px-6 bg-[#1a3a2a] text-white border-none rounded-lg font-outfit font-medium text-sm cursor-pointer transition-colors duration-200 hover:bg-[#132d20] disabled:opacity-50 disabled:cursor-not-allowed"
									>
										{issueMutation.isPending ? 'Issuing…' : 'Issue Certificate'}
									</button>
									<button
										onClick={() => navigate('/certifications')}
										className="py-2.5 px-6 bg-transparent text-[#44403c] border border-[#d6d3d1] rounded-lg font-outfit font-medium text-sm cursor-pointer transition-colors duration-200 hover:bg-[#f5f5f4]"
									>
										Cancel
									</button>
								</div>
						...
						<div className="w-105 shrink-0 sticky top-8">
							<p className="font-outfit text-[13px] font-medium text-[#78716c] text-center mb-3 uppercase tracking-[0.5px]">Preview</p>
							<div className="bg-white w-full aspect-[210/297] shadow-[0_10px_25px_rgba(0,0,0,0.15)] p-4 relative overflow-hidden">
								<div className="w-full h-full border-[5px] border-double border-[#2d7d4e] p-6 flex flex-col justify-between text-center">
									<div>
										<img src="/forest-3d-fluency-96.png" alt="SFC" className="w-14 h-14 object-contain mx-auto mb-3" />
