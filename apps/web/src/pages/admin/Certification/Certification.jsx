				...
				<main className="flex-1 p-4 sm:p-6 lg:p-8 flex flex-col gap-6 overflow-y-auto">
					...
					<section className="bg-white border border-[#e7e5e4] rounded-xl overflow-hidden">
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-b border-[#f5f5f4]">
							...
							<span className="font-outfit text-xs font-medium text-[#a8a29e]">{filtered.length} certificate{filtered.length !== 1 ? 's' : ''}</span>
						</div>

						<div className="overflow-x-auto">
							{isLoading && <p className="text-center py-12 font-outfit text-sm text-[#a8a29e]">Loading certifications…</p>}
							{error && <p className="text-center py-12 font-outfit text-sm text-red-500">Failed to load certifications.</p>}
							{!isLoading && !error && (
								<table className="w-full border-collapse">
									<thead>
										<tr>
											...
											<th className="bg-[#fafaf9] px-6 py-3.25 font-outfit text-xs font-semibold text-[#78716c] text-left tracking-[0.3px] uppercase whitespace-nowrap border-b border-[#f5f5f4]">Issued</th>
											<th className="bg-[#fafaf9] px-6 py-3.25 font-outfit text-xs font-semibold text-[#78716c] text-left tracking-[0.3px] uppercase whitespace-nowrap border-b border-[#f5f5f4]">Expires</th>
											<th className="bg-[#fafaf9] px-6 py-3.25 font-outfit text-xs font-semibold text-[#78716c] text-left tracking-[0.3px] uppercase whitespace-nowrap border-b border-[#f5f5f4]">Cert ID</th>
										...
										{filtered.length > 0 ? filtered.map((c, idx) => (
											<tr key={c.id} className={`hover:bg-[#fef7f0] ${idx === filtered.length - 1 ? '[&>td]:border-b-0' : ''}`}>
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle">
													<p className="font-serif text-[15px] text-[#1a3a2a]">{c.guide?.firstName} {c.guide?.lastName}</p>
												</td>
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle font-serif text-sm text-[#78716c]">{c.enrolment?.module?.title ?? '—'}</td>
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle font-serif text-sm text-[#78716c]">{new Date(c.issuedAt).toLocaleDateString()}</td>
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle font-serif text-sm text-[#78716c]">{c.expiresAt ? new Date(c.expiresAt).toLocaleDateString() : '—'}</td>
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle">
													<span className="font-outfit text-xs font-medium text-[#44403c] tracking-[0.3px]">{c.certificateNumber}</span>
												</td>
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle">
													<div className="flex items-center gap-3">
														<button
															className="inline-flex items-center py-1.25 px-3.5 border border-[#e7e5e4] rounded-md bg-[#fafaf9] font-outfit text-xs font-medium text-[#44403c] cursor-pointer transition-all duration-150 hover:bg-[#f0e9db] hover:border-[#d6d3d1]"
															onClick={() => handleDownload(c.id)}
														>
															PDF
														</button>
														<button
															className="inline-flex items-center gap-1 font-outfit text-[13px] font-medium text-[#b35c2a] bg-transparent border-none cursor-pointer p-0 transition-[gap] duration-200 hover:gap-2 hover:underline"
											...
											<tr>
												<td colSpan="6" className="text-center px-6 py-12 font-serif text-base text-[#a8a29e]">No certifications found.</td>
											</tr>
										)}
									</tbody>
								</table>
