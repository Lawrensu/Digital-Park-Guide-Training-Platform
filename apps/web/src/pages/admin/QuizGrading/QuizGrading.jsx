...
export default function QuizGradingPage() {
	...
	return (
		<div className="flex flex-col lg:flex-row min-h-screen bg-[#fdfbf7]">
			...
			{failToast && (
				<div className="fixed top-5 right-5 z-50 bg-[#b91c1c] text-white font-outfit text-[13.5px] font-medium py-3 px-5 rounded-xl shadow-lg max-w-sm">
			...
			<div className="flex-1 flex flex-col min-w-0">
				...
				<main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
					...
					<div className="flex items-center gap-4 mb-8">
						<button onClick={() => navigate('/quiz-reviews')} className="font-outfit text-sm text-[#78716c] hover:text-[#1a3a2a] transition-colors">
							← Back
						</button>
					</div>
					...
					<div className="grid grid-cols-1 lg:grid-cols-[2.5fr_1fr] gap-8 items-start">
						...
						<div className="flex flex-col gap-6">
							...
							<div className="flex justify-between items-start gap-8">
								<div>
									<h2 className="font-outfit m-0 mb-2 text-2xl font-semibold text-[#1a3a2a]">Grading: {attempt.quiz?.title ?? '—'}</h2>
									<div className="flex items-center gap-4">
										<span className="font-outfit bg-[#e8f5ee] text-[#266841] py-0.5 px-2 rounded text-xs font-semibold">Attempt #{attempt.attemptNumber ?? 1}</span>
										<span className="font-outfit text-[#78716c] text-[13px]">Submitted {new Date(attempt.submittedAt).toLocaleDateString()}</span>
									</div>
								</div>
								...
								<div className={`bg-white border border-[#e7e5e4] rounded-xl p-6 min-w-55 text-center border-l-[5px] ${isPass ? 'border-l-[#2d7d4e]' : 'border-l-[#d32f2f]'}`}>
									<div className="flex justify-between items-center mb-2">
										<span className="font-outfit text-xs text-[#78716c] font-semibold uppercase">Total Score</span>
										<span className={`font-outfit text-[11px] py-0.5 px-2 rounded-full font-bold ${isPass ? 'bg-[#e8f5ee] text-[#266841]' : 'bg-[#ffebee] text-[#d32f2f]'}`}>
											{isPass ? 'PASS' : 'FAIL'}
										</span>
									</div>
									<div className="font-outfit text-[40px] font-bold text-[#1c1917] leading-none mb-1">
										{totalScore} <span className="text-2xl text-[#a8a29e] font-normal">/ {maxTotal}</span>
									</div>
									<div className="font-outfit text-[13px] text-[#78716c]">Pass Mark: {passMark}</div>
								</div>
							</div>
								...
								{answers.length > 0 ? answers.map((answer, index) => (
									<div key={answer.questionId} className="bg-white border border-[#e7e5e4] rounded-xl p-6">
										...
										<div className="flex items-center justify-end gap-3">
											<label className="font-outfit text-xs font-semibold text-[#78716c] uppercase">Points:</label>
											<input
												type="number"
									...
									<p className="font-serif text-sm text-[#a8a29e] text-center py-8">No answers to grade.</p>
								)}
							</div>

							{submitError && <p className="font-outfit text-xs text-red-500">{submitError}</p>}
						...
						<div className="sticky top-8">
							<div className="bg-white border border-[#e7e5e4] rounded-xl p-6">
								<h3 className="font-outfit mt-0 mb-5 text-base font-semibold text-[#1c1917] border-b border-[#f5f5f4] pb-3">Attempt Summary</h3>
								...
								{[
									...
									<div key={label} className="flex justify-between mb-4">
										<span className="font-outfit text-sm text-[#78716c] font-medium">{label}</span>
										<span className="font-outfit text-sm text-[#1c1917] font-semibold text-right max-w-35">{value}</span>
									</div>
								))}

								<div className="h-px bg-[#f5f5f4] my-4" />

								<p className="font-outfit text-xs text-[#78716c] leading-[1.5] m-0">
