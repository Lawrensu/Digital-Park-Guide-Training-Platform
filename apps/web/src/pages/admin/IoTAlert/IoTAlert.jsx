				...
				<main className="flex-1 p-8 flex flex-col gap-5 overflow-y-auto">
					...
					<div className="flex gap-2">
						{TABS.map(tab => (
							<button
								key={tab}
								className={`py-[7px] px-5 rounded-full [font-family:var(--font-outfit)] text-[13px] font-medium border border-[#d6d3d1] bg-white text-[#44403c] cursor-pointer transition-all duration-150 hover:border-[#1a3a2a] hover:text-[#1a3a2a] ${activeTab === tab ? 'bg-[#1f4d35] border-[#1f4d35] text-white hover:text-white' : ''}`}
					...
					{isLoading && <p className="text-center py-12 [font-family:var(--font-outfit)] text-sm text-[#a8a29e]">Loading alerts…</p>}
					{error && <p className="text-center py-12 [font-family:var(--font-outfit)] text-sm text-red-500">Failed to load alerts.</p>}
					...
					{!isLoading && !error && (
						<div className="flex flex-col gap-3">
							{filtered.length > 0 ? filtered.map(alert => {
									...
									<div key={alert.id} className="bg-white border border-[#e5e7eb] border-l-4 rounded-xl py-4 px-5 flex flex-col gap-[6px] transition-shadow duration-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)]" style={{ borderLeftColor: style.border }}>
										<div className="flex items-start justify-between gap-3">
											...
											<div className="flex items-center gap-3 shrink-0">
												<span
													className="inline-flex items-center py-1 px-3 rounded-full [font-family:var(--font-outfit)] text-xs font-medium whitespace-nowrap"
													style={{ backgroundColor: style.bg, color: style.text }}
												>
													{SEVERITY_LABEL[alert.severity] ?? alert.severity}
												</span>
												<span className="[font-family:var(--font-outfit)] text-xs text-[#a8a29e] whitespace-nowrap">{new Date(alert.triggeredAt).toLocaleString()}</span>
											</div>
										</div>
								...
								<div className="text-center px-6 py-12 [font-family:var(--font-serif)] text-base text-[#a8a29e]">No alerts found for this filter.</div>
							)}
						</div>
					)}
