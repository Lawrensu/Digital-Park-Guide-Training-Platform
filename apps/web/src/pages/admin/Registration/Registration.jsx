		...
		<div className="flex flex-col lg:flex-row min-h-screen bg-[#fdfbf7]">
			...
			<div className="flex-1 flex flex-col min-w-0">
				<header className="flex items-center justify-between px-4 sm:px-8 h-16 bg-white border-b border-[#e7e5e4] shrink-0">
					<h1 className="font-outfit text-[20px] font-semibold text-[#1c1917]">Registrations</h1>
					<div className="flex items-center gap-3">
						...
						<div className="w-9 h-9 rounded-full bg-[#2d7d4e] flex items-center justify-center font-outfit text-xs font-semibold text-white" aria-label="Admin">AM</div>
					</div>
				</header>
				...
				<main className="flex-1 p-4 sm:p-6 lg:p-8 flex flex-col gap-6 overflow-y-auto">
					...
					<section className="bg-white border border-[#e7e5e4] rounded-xl overflow-hidden">
						<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-b border-[#f5f5f4]">
							...
							<span className="font-outfit text-xs font-medium text-[#a8a29e]">{filtered.length} result{filtered.length !== 1 ? 's' : ''}</span>
						</div>

						<div className="overflow-x-auto">
							{isLoading && (
								<p className="text-center py-12 font-outfit text-sm text-[#a8a29e]">Loading registrations…</p>
							)}
							{error && (
								<p className="text-center py-12 font-outfit text-sm text-red-500">Failed to load registrations.</p>
							)}
							{!isLoading && !error && (
								<table className="w-full border-collapse">
									<thead>
										<tr>
											...
											<th className="bg-[#fafaf9] px-6 py-3.25 font-outfit text-xs font-semibold text-[#78716c] text-left tracking-[0.3px] uppercase whitespace-nowrap border-b border-[#f5f5f4]">IC / Passport</th>
										...
										{filtered.length > 0 ? filtered.map((r, idx) => (
											<tr key={r.id} className={`hover:bg-[#fef7f0] ${idx === filtered.length - 1 ? '[&>td]:border-b-0' : ''}`}>
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle">
													<p className="font-serif text-[15px] font-normal text-[#1a3a2a]">{r.firstName} {r.lastName}</p>
												</td>
												...
												<td className="px-6 py-4.5 border-b border-[#f5f5f4] align-middle">
													<button className="inline-flex items-center gap-1 font-outfit text-[13px] font-medium text-[#b35c2a] bg-transparent border-none cursor-pointer p-0 transition-[gap] duration-200 hover:gap-2 hover:underline" onClick={() => navigate(`/registrations/${r.id}`)}>
											...
											<tr>
												<td colSpan="6" className="text-center px-6 py-12 font-serif text-base text-[#a8a29e]">No registrations found.</td>
											</tr>
										)}
									</tbody>
								</table>
