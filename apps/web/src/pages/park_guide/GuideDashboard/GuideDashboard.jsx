...
const MODULE_PROGRESS_TEXT_CLASS = {
	green:  '[font-family:var(--font-outfit)] text-[0.78rem] font-semibold text-[#2d7d4e]',
	blue:   '[font-family:var(--font-outfit)] text-[0.78rem] font-semibold text-[#3b82f6]',
	orange: '[font-family:var(--font-outfit)] text-[0.78rem] font-semibold text-[#f97316]',
}
				...
				<main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto flex flex-col gap-[1.75rem]">
					...
					<div className="relative bg-[#1a3a2a] rounded-[12px] py-8 px-5 sm:px-10 overflow-hidden">
						...
						<div className="relative z-[1] flex items-center gap-3.5">
							...
							<span className="[font-family:var(--font-outfit)] text-[0.8rem] font-medium text-white/80 whitespace-nowrap">{overallPct}% Overall Progress</span>
						</div>
					</div>
						...
						{stats.map((stat, i) => (
							<div key={i} className={`bg-white border border-[#e7e5e4] border-t-[3px] rounded-[10px] py-5 px-6 flex items-center gap-4 shadow-[0_1px_3px_rgba(0,0,0,0.05)] ${STAT_CARD_BORDER[stat.color]}`}>
								...
								<div className="flex flex-col gap-[3px]">
									<span className="[font-family:var(--font-outfit)] text-[0.9rem] font-semibold text-[#1c1917]">{stat.label}</span>
									<span className="[font-family:var(--font-outfit)] text-[0.78rem] text-[#78716c]">{stat.sub}</span>
								</div>
							</div>
						))}
					</div>
					...
					<div>
						<div className="flex items-center justify-between mb-4">
							<h3 className="[font-family:var(--font-outfit)] text-[1.05rem] font-semibold text-[#1c1917]">Modules In Progress</h3>
							<button
								onClick={() => navigate('/guide/modules')}
								className="[font-family:var(--font-outfit)] text-[0.82rem] font-medium text-[#2d7d4e] bg-transparent border-0 cursor-pointer hover:text-[#1a3a2a] hover:underline"
							>
								View all →
							</button>
						</div>
						...
						{modulesInProgress.length > 0 ? (
								...
								{modulesInProgress.map((enrolment, i) => {
									...
									return (
										<div key={enrolment.id} className={`bg-white border border-[#e7e5e4] border-l-4 rounded-[10px] p-5 flex flex-col gap-[0.65rem] shadow-[0_1px_3px_rgba(0,0,0,0.05)] ${MODULE_CARD_BORDER[color]}`}>
											...
											<div className="flex items-center justify-between">
												<span className="[font-family:var(--font-outfit)] text-[0.78rem] text-[#78716c]">{dueText}</span>
											</div>
											<button
												onClick={() => navigate(`/guide/modules/${enrolment.moduleId}`)}
												className="w-full py-[0.55rem] bg-[#1f4d35] text-white border-0 rounded-[6px] [font-family:var(--font-outfit)] text-[0.85rem] font-medium cursor-pointer transition-colors duration-[150ms] mt-1 hover:bg-[#1a3a2a]"
											>
												Continue →
											</button>
										</div>
							...
							<p className="[font-family:var(--font-outfit)] text-[0.9rem] text-[#78716c] py-4">
					...
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
						...
						<div className="bg-white border border-[#e7e5e4] rounded-[10px] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
							<div className="flex items-center justify-between mb-4">
								<h3 className="[font-family:var(--font-outfit)] text-[1.05rem] font-semibold text-[#1c1917]">Upcoming Due Dates</h3>
							</div>
							{upcoming.length > 0 ? (
									...
									{upcoming.map((enrolment, i) => {
										...
										return (
											<div key={enrolment.id} className="flex items-center gap-4 py-3 border-b border-[#f5f5f4] last:border-b-0">
												...
												<button
													onClick={() => navigate(`/guide/modules/${enrolment.moduleId}`)}
													className={`border-0 rounded-[6px] py-[0.45rem] px-[0.85rem] [font-family:var(--font-outfit)] text-[0.82rem] font-semibold cursor-pointer whitespace-nowrap transition-opacity duration-[150ms] hover:opacity-85 ${isUrgent ? 'bg-[#ef4444] text-white' : 'bg-[#1f4d35] text-white'}`}
												>
													Continue →
												</button>
											</div>
								...
								<p className="[font-family:var(--font-outfit)] text-[0.88rem] text-[#78716c]">No upcoming due dates set.</p>
							)}
						</div>

						<div className="bg-white border border-[#e7e5e4] rounded-[10px] p-6 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
							<div className="flex items-center justify-between mb-4">
								<h3 className="[font-family:var(--font-outfit)] text-[1.05rem] font-semibold text-[#1c1917]">My Certificates</h3>
								<button
							...
							{recentCerts.length > 0 ? (
										...
										return (
											<div key={cert.id} className="flex items-start gap-4 p-3 bg-[#fafaf9] border border-[#e7e5e4] rounded-[8px]">
												...
												<button
													...
													className="bg-[#1f4d35] text-white border-0 rounded-[6px] py-[0.45rem] px-3 [font-family:var(--font-outfit)] text-[0.78rem] font-semibold cursor-pointer whitespace-nowrap transition-colors duration-[150ms] self-center hover:bg-[#1a3a2a]"
												>
													📄 PDF
												</button>
											</div>
								...
								<p className="[font-family:var(--font-outfit)] text-[0.88rem] text-[#78716c]">No certificates yet. Complete a module to earn one.</p>
							)}
						</div>
