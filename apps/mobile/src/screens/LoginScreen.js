...
export default function LoginScreen() {
	...
	const handlePickCV = async () => {
		try {
			...
			if (!result.canceled && result.assets?.length > 0) {
				updateJoinForm('cv', result.assets[0].name);
			}
		} catch {
	...
	const handleJoinSubmit = () => {
		...
		const recipient = '102789275@students.swinburne.edu.my';
		...
		const body = encodeURIComponent(
			...
			`Email: ${joinForm.email}\nCV: ${joinForm.cv ? joinForm.cv + ' (attached separately)' : 'Will be sent separately.'}\n\n` +
			`Thank you.\n\n${joinForm.fullName}`
		);
			...
			<Modal visible={joinModal} transparent animationType="slide">
				<View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }}>
					<View style={{ backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '92%' }}>
						<View style={{
							backgroundColor: '#15803d', borderTopLeftRadius: 24, borderTopRightRadius: 24,
							...
							<TouchableOpacity
								onPress={() => { setJoinModal(false); setJoinErrors({}); }}
						...
						<ScrollView style={{ padding: 24 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
							...
							<View style={{ marginBottom: 14 }}>
								...
								<TouchableOpacity
									...
									style={{
										borderWidth: 1.5, borderColor: joinForm.cv ? '#15803d' : '#e5e7eb',
